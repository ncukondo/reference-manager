...
- **Server Mutation Persistence (#93)**: Mutations made via the HTTP server now survive restart.
  ...
  - `Library` now tracks a private `dirty` flag set by `add`/`update`/`remove` and cleared by `save()`/`reload()`. `dispose()` short-circuits `save()` when the library is clean, avoiding a redundant disk rewrite on mutation-free shutdowns (PR #94 review2 #3).
  ...
  - `ref server stop` now polls `process.kill(pid, 0)` after sending `SIGTERM` and only reports "Server stopped successfully" once the server PID has actually exited. If the PID is still alive after a 5s timeout, a stderr warning naming the pid and timeout is emitted instead — so a hung shutdown is visible rather than silently reported as success (PR #94 review2 #4).
  - `AddReferenceResult` no longer carries a tautological `added: true` literal — `addReference()` throws on failure, so the flag always held the same value and was asymmetric with `UpdateResult.updated` / `RemoveResult.removed` (PR #94 review2 #2).
  - `POST /api/references/` now returns `500` on `library.save()` I/O failures (ENOSPC, EACCES, etc.) instead of misreporting them as `400 "Invalid request body"`. JSON parse/validation errors still produce a `400` (PR #94 review3 #1).
  - `Library.save()` now derives `currentHash` from the in-memory serialized bytes rather than re-reading the file after writing, closing a race where an external writer landing inside the save window could poison `currentHash` and cause a later `reload()` to silently skip the external change as a "self-write" (PR #94 review3 #2).
  - `waitForPidExit` now returns a tagged `{status: 'exited' | 'timeout' | 'unreachable', ...}` result and `serverStop` uses it to write an accurate warning: a post-SIGTERM EPERM (or similar) probe failure now produces "could not confirm … unreachable (EPERM)" instead of the misleading "did not exit within {timeoutMs}ms" (PR #94 review3 #3).
  - `runShutdown` now guards `server.close()` and `removePortfile()` with try/catch. On failure each writes to stderr and sets `process.exitCode = 1`, but subsequent stages still run so a `close()` failure does not skip the save and a dispose failure does not leave a stale portfile (PR #94 review3 #4).
  - Test hardening: the external-file-change watcher tests (`src/mcp/context.test.ts` and `src/server/index.test.ts`) now poll until the reloaded state is visible rather than sleeping for a fixed 50 ms — fixes a flake that could fire on slow shared machines where the reload did not finish before the assertion (PR #94 review3 #5).
