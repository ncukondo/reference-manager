...
生成日: 2025-12-13 (最終更新: 2025-12-15 - Phase 3.3 仕様確定、created_at/timestamp導入)
...
#### 3.3 3-wayマージ 🔄 仕様確定 (2025-12-15)

**目標**: Last-Write-Wins (LWW) 戦略による競合検出と自動解決

| コンポーネント | ファイル | 状態 | 説明 |
|--------------|---------|------|------|
| Core Types Update | `src/core/csl-json/types.ts` | ⏳ 要修正 | `created_at` + `timestamp` 対応 |
| UUID Migration | `src/core/identifier/uuid.ts` | ⏳ 要修正 | レガシーマイグレーション対応 |
| Reference Update | `src/core/reference.ts` | ⏳ 要修正 | `getCreatedAt()`, `touch()` 追加 |
| Merge Types | `src/features/merge/types.ts` | ❌ 未実装 | マージ結果、競合、LWWの型定義 |
| Three-way Merge | `src/features/merge/three-way.ts` | ❌ 未実装 | LWW対応3-wayマージロジック |
| Merge Test | `src/features/merge/three-way.test.ts` | ❌ 未実装 | LWW戦略のテスト |
...
2. **3-wayマージ機能実装** (TDD):
   - `src/features/merge/types.ts` - マージ関連型定義
   - `src/features/merge/three-way.test.ts` - テスト先行実装 (LWW戦略)
...
**🔄 Phase 3.3: 3-way Merge - 既存コード修正完了、マージ実装準備完了** (2025-12-15)
...
2. ✅ **既存コード修正完了** (created_at/timestamp 移行):
   ...
   - ✅ `src/core/identifier/uuid.ts` - レガシーマイグレーション実装
     - `ensureCreatedAt()` - 作成日時確保、レガシー移行
     - `ensureTimestamp()` - 更新日時確保
     - `ensureCustomMetadata()` - 統合処理
   - ✅ `src/core/reference.ts` - 新メソッド追加
     - `getCreatedAt()` - 作成日時取得
     - `getTimestamp()` - 更新日時取得 (既存メソッド、意味変更)
     - `touch()` - 更新日時を現在時刻に更新
   - ✅ 全既存テスト修正 (360テスト全合格)
     - テストフィクスチャ8ファイル更新
     - library.test.ts, validator.test.ts修正

3. ⏳ **3-wayマージ機能実装** (TDD) - 次のステップ:
   - ⏳ `src/features/merge/types.ts` - 型定義
   - ⏳ `src/features/merge/three-way.test.ts` - テスト先行実装
   - ⏳ `src/features/merge/three-way.ts` - LWW対応マージロジック
   - ⏳ `src/features/merge/index.ts` - エクスポート
...
- **🔄 Phase 3: 機能モジュール - 部分完了** (最終更新: 2025-12-15)
  ...
  - ✅ Phase 3.2: 重複検出 - 完了 (2025-12-14)
    ...
    - **Phase 3.2 全24テスト合格** ✅
  - 🔄 Phase 3.3: 3-wayマージ - 既存コード修正完了 (2025-12-15)
    - ✅ Spec更新完了 (data-model.md, write-safety.md)
    - ✅ `created_at` + `timestamp` 戦略確定・実装完了
    - ✅ LWW (Last-Write-Wins) マージ戦略確定
    - ✅ 既存コード修正完了 (created_at/timestamp 移行、360テスト全合格)
    - ⏳ マージ機能実装 (次のステップ)
...
**総テスト数**: 360テスト合格 (Phase 1: 140, Phase 2: 77, Phase 3.1: 95, Phase 3.2: 24, その他: 24)

**現在の作業**: Phase 3.3 (3-wayマージ) - 既存コード修正完了、マージ機能実装準備完了
- ✅ Spec更新完了 (`data-model.md`, `write-safety.md`, `ROADMAP.md`)
- ✅ 既存コード修正完了 (`created_at`/`timestamp` 移行、360テスト全合格)
  - ✅ Core型定義更新 (`CslCustom`: `created_at` + `timestamp`)
  - ✅ レガシーマイグレーション実装 (旧`timestamp` → `created_at`)
  - ✅ Reference class 更新 (`getCreatedAt()`, `touch()`)
  - ✅ 全テストフィクスチャ・テストコード修正
- 次: マージ機能実装 (TDD - types → test → implementation)