...
2. Verify the SHA256 of the download against the release's `SHA256SUMS` asset. Mismatch is fatal and discards the download. If `SHA256SUMS` cannot be retrieved or has no entry for the asset (releases published before checksums existed), verification is skipped with a notice on stderr.
3. Verify the download: run `{tmp} --version` and confirm non-empty output matching `vX.Y.Z` (never executed if the checksum mismatched)
4. `chmod +x {tmp}`
5. `mv {tmp} {dest}` — POSIX `rename()` overwrites the destination atomically. No `rm` beforehand: deleting first would open a crash window with no binary installed at all.

The `SHA256SUMS` asset is generated by the release workflow (`.github/workflows/release-binary.yml`) with `sha256sum ref-*` over all built binaries.
...
| SHA256 checksum mismatch | Error; discard the download; never execute it |
| `SHA256SUMS` asset absent (older release) | Skip verification; print a notice to stderr |
