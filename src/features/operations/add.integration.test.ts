...
ER  - `,
  ...
  describe("ID collision resolution", () => {
    it("should preserve original ID from csl-json file", async () => {
      const jsonPath = path.join(testDir, "custom-id.json");
      await fs.writeFile(
        jsonPath,
        JSON.stringify([
          {
            id: "my-custom-citation-key",
            type: "article-journal",
            title: "Article with Custom Key",
            author: [{ family: "Smith" }],
            issued: { "date-parts": [[2024]] },
          },
        ]),
      ...
      expect(result.added[0].id).toBe("my-custom-citation-key");
      expect(result.added[0].idChanged).toBeUndefined();
      expect(result.added[0].originalId).toBeUndefined();
    });

    it("should resolve original ID collision and report change", async () => {
      // Add existing reference
      await library.add({
        id: "smith2024",
        type: "article-journal",
      ...
      // Add new reference with same original ID
      ...
      expect(result.added).toHaveLength(1);
      // Original ID collision resolved with suffix
    ...
    it("should NOT report idChanged when item has no original ID (empty string)", async () => {
      // Add existing reference that will cause generated ID collision
      await library.add({
        id: "smith-2024",
        type: "article-journal",
        title: "Completely Different Title for First Article",
        author: [{ family: "Smith" }],
        issued: { "date-parts": [[2024]] },
        DOI: "10.1000/first",
      });
      ...
      // Add new reference with empty ID (will be generated from author-year)
      // CSL-JSON schema requires id field, so we use empty string to simulate "no original ID"
      // Use completely different title to avoid duplicate detection
      const jsonPath = path.join(testDir, "empty-id.json");
      await fs.writeFile(
        jsonPath,
        JSON.stringify([
          {
            id: "", // Empty string = no original ID
            type: "article-journal",
            title: "Another Unique Title for Testing ID Generation",
            author: [{ family: "Smith" }],
            issued: { "date-parts": [[2024]] },
            DOI: "10.1000/second",
          },
      ...
      expect(result.added).toHaveLength(1);
      // Generated ID collision resolved with suffix
      expect(result.added[0].id).toBe("smith-2024a");
      // Generated ID collision should NOT be reported
      expect(result.added[0].idChanged).toBeUndefined();
      expect(result.added[0].originalId).toBeUndefined();
    });

    it("should preserve original ID from bibtex file", async () => {
      const bibPath = path.join(testDir, "custom.bib");
      await fs.writeFile(
        bibPath,
        `@article{MyCustomBibtexKey2024,
  title = {Article with Custom BibTeX Key},
  author = {Doe, John},
  year = {2024}
}`,
        "utf-8"
      ...
      expect(result.added).toHaveLength(1);
      // Original bibtex key should be preserved
      expect(result.added[0].id).toBe("MyCustomBibtexKey2024");
      expect(result.added[0].idChanged).toBeUndefined();
      expect(result.added[0].originalId).toBeUndefined();
    });

    it("should resolve bibtex key collision and report change", async () => {
      // Add existing reference with same key
      await library.add({
        id: "DuplicateKey2024",
        type: "article-journal",
        title: "First Article",
      ...
      const bibPath = path.join(testDir, "duplicate.bib");
      await fs.writeFile(
        bibPath,
        `@article{DuplicateKey2024,
  title = {Second Article with Same Key},
  author = {Jones, Bob},
  year = {2024}
}`,
        "utf-8"
      ...
      expect(result.added).toHaveLength(1);
      // Original bibtex key collision resolved with suffix
      expect(result.added[0].id).toBe("DuplicateKey2024a");
      // Collision should be reported
      expect(result.added[0].idChanged).toBe(true);
      expect(result.added[0].originalId).toBe("DuplicateKey2024");
    });
  });
