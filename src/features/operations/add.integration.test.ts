  ...
  describe("End-to-end file import", () => {
    describe("JSON files", () => {
      it("should import single reference from JSON file", async () => {
        ...
        // generateId creates author-year format: smith-2024
      ...
      it("should import multiple references from JSON file", async () => {
        ...
        // generateId creates author-year format
        expect(result.added.map((r) => r.id)).toEqual([
          "author1-2021",
          "author2-2022",
          "author3-2023",
        ]);
      ...
      it("should import from multiple JSON files", async () => {
        ...
        // generateId creates author-year format
        expect(result.added.map((r) => r.id)).toContain("firstauthor-2020");
        expect(result.added.map((r) => r.id)).toContain("secondauthor-2021");
      });
    });
...
ER  - `,
  ...
  describe("ID collision resolution", () => {
    it("should resolve ID collision by appending suffix", async () => {
      // Add existing reference with smith-2024 ID (lowercase, as generated by generateId)
      await library.add({
        id: "smith-2024",
        type: "article-journal",
      ...
      // Add new reference with same author+year (generates same ID) but different title
      ...
      expect(result.added).toHaveLength(1);
      // generateId creates smith-2024, collision resolved to smith-2024a
      expect(result.added[0].id).toBe("smith-2024a");
      expect(result.added[0].idChanged).toBe(true);
      expect(result.added[0].originalId).toBe("smith-2024");
    });
  });
