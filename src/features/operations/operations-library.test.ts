...
import type { ILibrary, UpdateOptions, UpdateResult, RemoveResult } from "../../core/library-interface.js";
...
describe("OperationsLibrary", () => {
	...
	describe("high-level operations", () => {
		...
		it("should call searchReferences with format option", async () => {
			...
			expect(searchReferences).toHaveBeenCalledWith(mockLibrary, { query: "Smith", format: "ids-only" });
			expect(result).toBe(searchResult);
		});
		...
		it("should call citeReferences for cite()", async () => {
			...
			const mockLibrary = createMockLibrary();
			const citeResult = { results: [{ identifier: "smith2023", found: true, citation: "Smith (2023)" }] };
		...
		it("should call addReferences with options", async () => {
			...
			expect(addReferences).toHaveBeenCalledWith(["10.1234/test"], mockLibrary, { force: true, format: "bibtex" });
			expect(result).toBe(addResult);
		});
	});
});
